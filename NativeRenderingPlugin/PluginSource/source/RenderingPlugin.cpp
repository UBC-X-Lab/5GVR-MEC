...
extern "C" void UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API SetupConnectionFromUnity(char* IP, char* port)
{
	...
	LOG("destinationIP, %s\n", IP);
	LOG("destinationPort, %s\n", port);
	
	destinationIP = std::string(IP);
	destinationPort = std::string(port);
}
...
DWORD WINAPI runServer(LPVOID param) {
	...
	iResult = getaddrinfo(destinationIP.c_str(), destinationPort.c_str(), &hints, &result);
	if (iResult != 0) {
		LOG("DestinationIP: %s\n", destinationIP);
		LOG("destinationPort: %s\n", destinationPort);
