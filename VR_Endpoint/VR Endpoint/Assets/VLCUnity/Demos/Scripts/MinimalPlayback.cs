...
public class MinimalPlayback : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        _libVLC = new LibVLC("--no-osd","--verbose=2","--low-delay");// "--file-caching=200", "--low-delay", "--clock-jitter=0"

        Application.SetStackTraceLogType(LogType.Log, StackTraceLogType.None);
        _libVLC.Log += (s, e) => UnityEngine.Debug.Log(e.FormattedLog); // enable this for logs in the editor
        Debug.Log("version= " + _libVLC.Version);
    ...
    public async void PlayPause()
    {
        ...
        else
        {
            ...
            if(_mediaPlayer.Media == null)
            {
                ...
                var media = new Media(_libVLC, "udp://@:10000", FromType.FromLocation);
                await media.Parse();
