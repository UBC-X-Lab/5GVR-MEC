...
public class OVRCameraRig : MonoBehaviour
{
	...
	public StreamWriter sw;  // writes to stream
	public StreamReader sr;  // reads from incoming stream
	private string headDataPath; // path to head data
	public Boolean useFakeHeadData; // indicates wether or not fake head data is to be used
	public Boolean recordFakeHeadData; // indicates if record should be kept of fake head data
...
#region Unity Messages
	...
	protected virtual void Start()
	{
		...
		if (recordFakeHeadData) // initailize streamwriter
		{
			sw = new StreamWriter(Application.persistentDataPath + @"\headsample.txt", true);
		}
		else if (useFakeHeadData) // initialize streamreader
		{
			sr = new StreamReader(Application.persistentDataPath + @"\headsample.txt");
		}
	}
	...
	protected virtual void UpdateAnchors(bool updateEyeAnchors, bool updateHandAnchors)
	{
		...
		if (updateEyeAnchors)
		{	
			if (recordFakeHeadData)
			{
				sw.WriteLine(centerEyeAnchor.localPosition.x + "," + centerEyeAnchor.localPosition.y + "," + centerEyeAnchor.localPosition.z + "," + centerEyeAnchor.localRotation.x + "," + centerEyeAnchor.localRotation.y + "," + centerEyeAnchor.localRotation.z + "," + centerEyeAnchor.localRotation.w);
				sw.Flush();
			...
			if (useFakeHeadData)
			{
                ...
                if (line == null)
                {
					sr = new StreamReader(headDataPath);
                }
                else
                {
					...
					Vector3 fakePosition = new Vector3(float.Parse(values[0]), float.Parse(values[1]), float.Parse(values[2]));
					Quaternion fakeRotation = new Quaternion(float.Parse(values[3]), float.Parse(values[4]), float.Parse(values[5]), float.Parse(values[6]));
					//Debug.Log(fakePosition.ToString() + fakeRotation.ToString());
					centerEyeAnchor.localPosition = fakePosition;
					centerEyeAnchor.localRotation = fakeRotation;
