...
int main(int argc, char **argv)
{
    ...
    frame->width = ctx->width; // ** Should probably use g_videoWidth from above
    frame->height = ctx->height; // ** same but with height
    ...
    while (ti.ready)
    {
        read_frame(frame);
        struct timeval tv;
        gettimeofday(&tv, NULL); // will be used to determine sleeptime
